...
| `verify` | `verify:routes && verify:responsive && verify:interactions` | The full Playwright verification suite, run against `preview` |
| `verify:routes` / `verify:responsive` / `verify:interactions` | `node scripts/verify-*.mjs` | See §8 |
| `audit:content` / `audit:a11y` | `node scripts/audit-*.mjs` | See §8 — reviewing aids, not part of `verify` |
...
| `CourseComplete.tsx` | `/course-complete` | A completion certificate / stats recap with "where to next" cards. | No hooks besides `useDocumentTitle`; `CERT`/`NEXT_PATHS`/`CONFETTI` are static — **not yet wired to `useProgress()`**, so the certificate stats aren't real. | `TopNav`, `Icon`, `Tag` | Both "next path" cards link to `/roadmap`. |
...
| `CliBasics.tsx` | `/cli-basics` | The flagship lesson: a clickable command-anatomy diagram, a self-checking quiz, and a terminal walkthrough. | `AnatomyKey` selection state; `answers: Record<number,number>` for the quiz; `step`/`revealed` for the walkthrough. All content comes from `data/cliBasics.ts`. | `ConceptSidebar`, `SectionHead`, `CodeListing`, `CopyPanel`, `Aside`, `Icon`, `ui`, `useCopy`, `useDocumentTitle`, `useProgress`, `data/cliBasics.ts` | Renders three `CopyPanel`s off one shared `useCopy()` call keyed `'bash'\|'ps'\|'pipes'`. Its `SIDEBAR` shape is duplicated (not imported) by `ShellScripting.tsx`. |
...
| `Roadmap.tsx` | `/roadmap` | A five-stage learning path: stage 1 done, stage 2 "you are here" with a chip grid, stages 3-5 locked. | Progress bar reads `useProgress().state.concepts` count (floored at `BASELINE_DONE`). | `TopNav`, `Meter`(`ui`), `useDocumentTitle`, `useProgress` | Stage-2 chips link to `/shell-scripting` and `/rebase-history` — the page those two are linked *from*. |
...
| `ProgressDashboard.tsx` | `/progress-dashboard` | Streaks, a 14-day minutes chart, quiz accuracy, a completion donut, badges, and an "up next" queue. | Imports `streakOf`, `recentMinutes`, `isDue` directly (not just the hook) to compute derived stats each render (not memoized). Reset button calls `useProgress().reset()`. | `TopNav`, `Meter`, `useDocumentTitle`, `lib/progress.ts` (several standalone helpers) | The single biggest consumer of `lib/progress.ts`. Links to `/flashcards`, `/quiz-mode`, `/git-branching`. |
...
| `Flashcards.tsx` | `/flashcards` | Flip cards, rate yourself, and the deck schedules real spaced-repetition reviews. | `index`/`flipped`/`rated` state; a `useMemo` (deliberately empty deps) freezes the due-card set for the session so rating doesn't reshuffle it. | `TopNav`, `useDocumentTitle`, `useProgress` (`rateCard` → `schedule()`) | Shares `state.cards` with `ProgressDashboard`, which links here from its "up next" queue. A session ends on a summary screen rather than looping. |
...
| `audit-a11y.mjs` | `audit:a11y` (not part of `verify`) | Checks WCAG text-contrast ratios and accessible names on every interactive control, across all `ROUTES`. Needs `npm run preview` since it drives a browser. | `browser.mjs`, `routes.mjs` |
