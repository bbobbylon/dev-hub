...
| `data/curriculum.ts` | `TOTAL_CONCEPTS`, `TOTAL_CHECKPOINTS` | The numbers the designs quote about the *course*, as opposed to what the app derives from real data. `TOTAL_CONCEPTS` was a bare `23` in three files. **Not `PAGES.length`** — a concept is a unit of the designed path, a page is one of ~26 screens, several of which teach no concept while CLI Basics walks through several. | `Roadmap`, `ProgressDashboard`, `CourseComplete` |
...
| `Roadmap.tsx` | `/roadmap` | A five-stage learning path: stage 1 done, stage 2 "you are here" with a chip grid, stages 3-5 locked. | Progress bar reads `useProgress().state.concepts` count (floored at `BASELINE_DONE`). | `TopNav`, `Meter`(`ui`), `useDocumentTitle`, `useProgress`, `data/curriculum.ts` | Stage-2 chips link to `/shell-scripting` and `/rebase-history` — the page those two are linked *from*. |
