...
_Last updated: 2026-09-21 — describes the 43-page app plus the optional `server/` backend, on the
`polish-content-gaps` branch. The five-stage, 25-concept designed backend-developer path is fully
built as of item 46 (Deploy); only the capstone (`Project Build-Along`, already built) remains._
...
see that root `README.md`). It ships ~43 standalone interactive lesson/tool pages behind one
...
└── app/                    The actual React app (everything below is under here)
    ...
    │   ├── lib/              Persistence, sync, and the sandboxed code runner — 6 files (see §4)
    │   ├── assets/
    │   │   └── recursion-poster.svg   Hand-drawn poster frame for Video Lesson, built from the app's own tokens
    │   └── pages/           One component per route — 47 files (see §7)
    └── scripts/             The Playwright verification/audit toolchain — 9 scripts + 1 baseline data file (see §8)
```
...
| --- | --- | --- | --- |
| `components/TopNav.tsx` | `TopNav` | The sticky top bar every page carries; brand mark links back to the gallery (`/`). Also reads `useAuth()` directly (not a prop threaded through ~30 call sites) to render the signed-in user's name + "Sign out" after the `right` slot, or a "Sign in" link — the latter only when `apiEnabled`, matching `App.tsx`'s conditional account routes. A signed-in user keeps the name + "Sign out" either way, so a token left by an earlier API-enabled build can still be cleared. | Every page |
...
| `components/ConceptComplete.tsx` | `ConceptComplete` | The end-of-lesson panel and the only place a concept gets recorded. Reads `slug` against `data/concepts.ts`, writes via `useProgress().completeConcept`. Two modes by design: pass `earned` and it records the moment the page's own finishing condition fires (guarded on the existing timestamp, so an old completion keeps its original date); omit it — as the static field-guide pages do, having no such moment — and it offers a button instead, with `hint` naming what would have earned it automatically. A completed panel offers the inverse, "Un-mark complete" (`clearConcept`), which sets a local flag suppressing the `earned` auto-record for as long as `earned` stays true — so un-marking sticks on a walkthrough already at its last step. That makes the `earned` contract explicit: it must describe a moment in *this visit*, never a state read back out of the store, or the next visit would silently undo the un-mark. Renders `aria-label="<label> completion"`, which `verify-interactions.mjs` uses to read a page's state with the panel subtracted. | Every lesson and practice page (18), plus `QuizMode`'s score screen |
...
| `components/CommandPalette.tsx` | `CommandPalette` | Global "jump to any page" dialog (BACKLOG item 12) — Cmd/Ctrl+K opens it from any route, arrow keys move the highlighted `data/pages.ts` `PAGES` match, Enter navigates, Escape/backdrop-click closes. Renders `null` while closed, so it adds nothing to any route's own a11y surface; uses the handoff's own `.dialog`/`.dialog-backdrop` classes (already in `organic.css`, previously unused anywhere in the app). | Mounted once in `App.tsx`, alongside (not inside) `<Routes>` |
...
| `data/gitBasicsQuiz.ts` | `QUESTIONS`, `Question`, `QUIZ_ID`, `PASS_MARK` | The Git Basics checkpoint's question bank, moved out of `QuizMode.tsx` (BACKLOG item 15) once `ProgressDashboard`'s "up next" queue needed `QUIZ_ID`/`PASS_MARK` too, to say whether the checkpoint still wants taking, retaking, or reviewing without a second copy of either literal. `QuizMode` still owns all the grading logic — this file is just the bank plus the two constants. | `QuizMode`, `ProgressDashboard` |
| `data/concepts.ts` | `CONCEPTS`, `CONCEPT_BY_SLUG`, `PATH_CONCEPTS`, `conceptsInStage`, `pathDone`, `offPathDone`, the `Concept` type | The registry of the 38 concepts the app can record as complete: slug (the key in `state.concepts`), label, the route that teaches it, its Roadmap stage or `null` when off the five-stage path, and prose for what earns it. Two path concepts carry no `route` — the design names them and no page teaches them, so `ProgressDashboard`'s "Your concepts" panel is the only place either can be marked (or un-marked). Counting goes through `pathDone`/`offPathDone` rather than `Object.keys(state.concepts).length`, so an off-path concept, a stale key from an older build, or a restored backup can't inflate a figure labelled "Backend path". Reference pages (`Glossary`, `CheatSheet`), `Flashcards` (its own SM-2 state) and the meta pages are deliberately absent. | `ConceptComplete`, `Roadmap`, `ProgressDashboard`, `data/curriculum.ts` |
| `data/curriculum.ts` | `TOTAL_CONCEPTS`, `TOTAL_CHECKPOINTS`, `UPCOMING_STAGES` | The numbers the designs quote about the *course*, as opposed to what the app derives from real data. `TOTAL_CONCEPTS` was a bare `23` in three files; it is now derived from `PATH_CONCEPTS` plus the not-yet-built stages' listed concepts, which corrected it to 25 — the Roadmap's prose said 23 while its own chips enumerated 25. **Not `PAGES.length`** — a concept is a unit of the designed path, a page is one of ~43 screens, several of which teach no concept while CLI Basics walks through several. Nor `CONCEPTS.length`, which includes the off-path pages. `syllabusOf` — the one-line syllabus a fully-locked stage used to print — was removed at item 41 alongside the `laterStages` machinery it served, once stage 5 (the last stage that ever needed it) got its first real concept. | `Roadmap`, `ProgressDashboard`, `CourseComplete` |
...
| `lib/sandboxRun.ts` | `runInSandbox`, `SandboxResult` | Runs a JS source string in a real Web Worker (its own realm, no DOM, no access to this page's state), capturing `console.log` calls as output lines; a run that throws or never settles resolves as a failure rather than hanging the page (BACKLOG item 9). Lets "Run tests" grade what code actually produces instead of a hardcoded pass — the prototype's exercise was Python pseudocode, translated to JavaScript here since a Worker can't run Python without a disproportionate WASM runtime. | `CodePlayground` only |
...
| `verify` | `node scripts/run-verify.mjs` | Starts and owns its own `vite preview` server, runs `verify:routes` → `verify:responsive` → `verify:interactions` → `audit:a11y` in sequence against it, then always tears it down. Needs `dist/` to exist (`npm run build` first). See §8. |
...
| --- | --- | --- | --- | --- | --- |
| `Http.tsx` | `/http` | Stage 5's 1st lesson — four graded predict-the-outcome questions on PUT idempotency, 401 vs. 403, PUT vs. PATCH, and statelessness, each an `.http` request/response transcript. | `answers` (per-question pick) drives score/reveal/pass-mark state; `<ConceptComplete earned={score >= 3}>` records `http`. | `TopNav`, `CodeListing`/`syn`, `Icon`, `Tag`, `useDocumentTitle`, `ConceptComplete` | Graduated Stage 5 out of `Roadmap.tsx`'s locked `laterStages` path; linked from the Roadmap's stage-5 block. |
| `Rest.tsx` | `/rest` | Stage 5's 2nd lesson — four graded predict-the-outcome questions on nested resource URLs, 404 vs. an empty body, query params vs. one path per view, and POST vs. PUT for creation. | Same score/reveal/pass-mark shape as `Http.tsx`; records `rest`. | Same as `Http.tsx` | Builds directly on `/http`'s statelessness/idempotency ground without repeating it. |
| `SqlBasics.tsx` | `/sql-basics` | Stage 5's 3rd lesson — four graded predict-the-outcome questions on NULL comparison, DELETE/UPDATE with no WHERE, COUNT(*) vs. COUNT(column), and WHERE vs. HAVING, as `.sql` transcripts. | Same score/reveal/pass-mark shape; records `sql-basics`. | Same as `Http.tsx`, `.sql` listings instead of `.http` | Deliberately kept single-table so `/joins` has untaught ground; the site's first SQL content. |
| `Joins.tsx` | `/joins` | Stage 5's 4th lesson — four graded predict-the-outcome questions on INNER vs. LEFT JOIN row counts, a WHERE clause silently undoing a LEFT JOIN, many-to-many fan-out, and LEFT JOIN + IS NULL, verified against a real SQLite engine. | Same score/reveal/pass-mark shape; records `joins`. | Same as `SqlBasics.tsx` | Its first question reuses `/sql-basics`' own dataset as a callback. |
| `Auth.tsx` | `/auth` | Stage 5's 5th lesson — four graded predict-the-outcome questions on why a fast hash is the wrong tool for a password, a JWT signed vs. encrypted, cookie CSRF vs. bearer-token XSS, and revoking a JWT that already shipped. | Same score/reveal/pass-mark shape; records `auth`. | Same as `Http.tsx` | Goes past `/http`'s own 401-vs-403 question into what proof of identity actually is. |
| `Deploy.tsx` | `/deploy` | Stage 5's 6th and final lesson — four graded predict-the-outcome questions on a client-bundle secret leak, build-time vs. runtime env vars, a rolling deploy's old/new overlap, and a migration surviving that overlap, as `.sh` deploy transcripts. | Same score/reveal/pass-mark shape; records `deploy`. | Same as `Http.tsx`, `.sh` listings | Completes Stage 5 and the whole five-stage path; only the capstone (`Project Build-Along`) remains. |
...
| `QuizMode.tsx` | `/quiz-mode` | A Git Basics exam flow: question palette, per-question feedback, score screen. | `index`/`selected`/`checked`/`results` step through `data/gitBasicsQuiz.ts`'s `QUESTIONS`; on completion, tallies correct/total per question `topic` and calls `useProgress().recordQuiz()` with that breakdown so `ProgressDashboard`'s "weakest topic" panel has real data. `git-basics` is recorded by a `<ConceptComplete earned={passed}>` below the score card rather than a bare `completeConcept()` call, so the checkpoint can be un-marked like any other concept; that panel renders only once an attempt has passed (or an earlier one did, which keeps the un-mark reachable after a failed retry), since the quiz — not the learner — is the authority here. | `TopNav`, `Tag`, `useDocumentTitle`, `useProgress`, `ConceptComplete`, `data/gitBasicsQuiz.ts` | Completion screen links to `/roadmap`. |
...
| `CourseComplete.tsx` | `/course-complete` | A completion certificate / stats recap with "where to next" cards. | The certificate's name, date, and capstone stat are computed in-component from `useAuth()`/`useProgress()` (BACKLOG item 7) — a signed-in learner's real name or "You", today's date, and whether `project-build-along` is actually recorded, rather than "Ada Moreno" / a fixed date / a fixed "1 capstone shipped". `NEXT_PATHS`/`CONFETTI`, the path title, and the certificate id stay static: this page isn't gated behind actually finishing a path (see items 24-25 on why stage 1 can't reach COMPLETE yet), so there's no real state to swap the path title from. | `TopNav`, `Icon`, `Tag`, `data/curriculum.ts`, `lib/auth.tsx`, `lib/progress.ts` | Both "next path" cards link to `/roadmap`. |
...
| `Roadmap.tsx` | `/roadmap` | A five-stage learning path, and as of item 46 the entire path is fully built — every stage's `concepts` array is empty, so no `NotBuiltChip` renders anywhere on the page: stage 3 and stage 4 are fully built (all six concepts each), and stage 5 is too (HTTP item 41, REST item 42, SQL Basics item 43, Joins item 44, Auth item 45, Deploy item 46). Stages 3-5 share one real `PartialStage` component — extracted at item 41, replacing the hand-duplicated JSX blocks items 35-40 kept in place — instead of the fully-locked `laterStages` path a not-yet-started stage used to render through; that machinery (`LockedTag`, `NumberNode`'s locked state, `syllabusOf`) was removed rather than left idle, since stage 5 was the last stage that could ever reach it. | Every chip state, stage badge, "next up" marker, Continue link and count is derived from `state.concepts` against `conceptsInStage()`; nothing is hardcoded, and the old `BASELINE_DONE = 8` floor is gone. Chips for concepts with no `route` render as a plain `<span>` rather than a `Link` even once marked done — that marking happens on `ProgressDashboard`'s "Your concepts" panel instead, since there's nowhere here to click through to. | `TopNav`, `Meter`(`ui`), `useDocumentTitle`, `useProgress`, `data/concepts.ts`, `data/curriculum.ts` | Stage-2 chips link to `/shell-scripting` and `/rebase-history` — the page those two are linked *from*. |
...
| `ProgressDashboard.tsx` | `/progress-dashboard` | Streaks, a 14-day minutes chart, quiz accuracy, a completion donut, badges, a "weakest topic" callout, an "up next" queue, a "Your concepts" panel (mark / un-mark any concept), and the "Your data" panel (export / import / reset). | Imports `streakOf`, `recentMinutes`, `isDue`, `weakestTopic` directly (not just the hook) to compute derived stats each render (not memoized). All four badges (BACKLOG item 15) now carry a real `earned` condition instead of two fixed `true`s and two fixed `false`s — Terminal Tamer off both routed Stage-1 concepts, 7-Day Flame off the real streak, Bug Hunter off `debugging-challenge`, First Path off the path percentage reaching 100 — and the two badges that used to print an invented stat ("2 of 5 cases", "34%") print a real one instead. The "up next" queue is three real recommendations (flashcards due, the checkpoint's take/retry/review state via `data/gitBasicsQuiz.ts`'s `QUIZ_ID`/`PASS_MARK`, and the next not-yet-complete lesson via `data/concepts.ts`) rather than a fixed template with only the flashcard count swapped in; the per-item minute estimates and the panel's "15 minutes total" header are gone rather than replaced, since no page records how long it takes. "Your concepts" (BACKLOG items 24 & 29) lists every `CONCEPTS` entry grouped like the Roadmap, each row calling `completeConcept`/`clearConcept` directly — the only way to un-mark a concept without returning to its page, and the only way to mark `environment-variables`/`staging-commits` at all, since neither has one. Export/import/reset call `downloadProgress`, `parseProgressFile` + `useProgress().importState`, and `reset()` respectively; a `useRef` file input sits behind the Import button, and one `note` state carries the result line under the row. | `TopNav`, `Meter`, `useDocumentTitle`, `lib/progress.ts` (several standalone helpers), `lib/progressFile.ts`, `lib/api.ts` + `lib/auth.tsx` (for the storage blurb), `data/concepts.ts`, `data/gitBasicsQuiz.ts` | The single biggest consumer of `lib/progress.ts`, and the only page that acts on the whole blob at once. Links to `/flashcards`, `/quiz-mode`, and the next incomplete lesson. |
...
| `ProjectBuildAlong.tsx` | `/project-build-along` | A capstone brief (log-watcher CLI): acceptance criteria, file-layout diagram, 5-item milestone checklist. | Milestones persist via `useProgress().toggleMilestone` under `"project-build-along:<index>"` keys; falls back to `INITIAL_DONE` until first interaction so it renders like the original mockup. Its `<ConceptComplete>` condition is the *transition* to a full checklist (`allDone && wasIncomplete`), not the persisted state itself — the only page where that distinction is needed, and what stops a revisit from re-recording a concept the learner just un-marked. | `TopNav`, `useDocumentTitle`, `useProgress`, `ConceptComplete` | Milestone text references CLI Basics, Regex Lab, and Git Basics concepts. |
...
| `CodePlayground.tsx` | `/code-playground` | A FizzBuzz code challenge: brief, read-only solution listing, test checklist. | "Run tests" really executes the exact solution source in a Web Worker (`lib/sandboxRun.ts`, its own realm, no DOM access — BACKLOG item 9) and grades each checklist item off the `console.log` output that run actually produced; `status`/`result` hold the run's state, not a scripted pass. | `TopNav`, `CodeListing`, `Icon`, `Tag`, `useDocumentTitle`, `ConceptComplete`, `lib/sandboxRun.ts` | Self-contained. |
...
| --- | --- | --- | --- |
| `run-verify.mjs` | `verify` | Owns the full run end to end (BACKLOG item 28): starts `vite preview` itself via Vite's JS API with `strictPort: true` (so a stale server left over from an earlier session fails loudly instead of silently serving from the wrong port while these suites keep checking 4173), runs the four suites below against it in sequence, then always tears the server down — success, failure, or a suite that throws. Requires `dist/` to already exist (`npm run build` first). `VERIFY_BASE_URL` opts out of all of this, for pointing the suites at a server it doesn't own. | `vite`'s `preview()` API, then spawns each suite below as a child process |
...
| `audit-a11y.mjs` | `audit:a11y` (the fourth leg of `verify`) | Checks WCAG text-contrast ratios and accessible names on every interactive control, across all `ROUTES`, then compares the result to `scripts/a11y-baseline.json` and fails only on a regression: a contrast pair not in the baseline, a rise in total failures with no new pair (a new page repeating an accepted bad pair), or any unnamed control. The inherited failures are real and accepted, so a plain pass/fail could only ever say "fail" — and a check that can never pass can't be gated on, which is how the count drifted 31 → 39 unnoticed the first time. Currently 35 failures across 19 pairs — see `app/README.md`'s "Accessibility" section for the full history, including the one genuine dark-ground contrast bug (`CodeListing`'s line-number gutter) that a 2026-09-16 investigation found and fixed, bringing it down from 39/20. `-- --update-baseline` re-records, deliberately and with a reviewable diff. Needs `npm run preview` since it drives a browser. | `browser.mjs`, `routes.mjs`, `a11y-baseline.json` |
