...
| `QuizMode.tsx` | `/quiz-mode` | A Git Basics exam flow: question palette, per-question feedback, score screen. | `index`/`selected`/`checked`/`results` step through `QUESTIONS`; on completion, calls `useProgress().recordQuiz()`. `git-basics` is recorded by a `<ConceptComplete earned={passed}>` below the score card rather than a bare `completeConcept()` call, so the checkpoint can be un-marked like any other concept; that panel renders only once an attempt has passed (or an earlier one did, which keeps the un-mark reachable after a failed retry), since the quiz — not the learner — is the authority here. | `TopNav`, `Tag`, `useDocumentTitle`, `useProgress`, `ConceptComplete` | Completion screen links to `/roadmap`. |
