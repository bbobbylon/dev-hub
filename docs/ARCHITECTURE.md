...
_Last updated: 2026-09-19 — describes the 37-page app plus the optional `server/` backend, on the
...
see that root `README.md`). It ships ~37 standalone interactive lesson/tool pages behind one
...
| `Roadmap.tsx` | `/roadmap` | A five-stage learning path: stages 1-4 built with real chip grids (stage 3 and stage 4 both fully built, all six concepts each — stage 4's last, Sorting, items 35-40: Arrays, Hash Maps, Stacks & Queues, Trees, Big-O, Sorting), stage 5 the only stage still a fully locked placeholder. Stage 3 and stage 4 share one hand-duplicated JSX shape rather than an extracted component — see the file's own header comment for why. | Every chip state, stage badge, "next up" marker, Continue link and count is derived from `state.concepts` against `conceptsInStage()`; nothing is hardcoded, and the old `BASELINE_DONE = 8` floor is gone. Chips for concepts with no `route` render as a plain `<span>` rather than a `Link` even once marked done — that marking happens on `ProgressDashboard`'s "Your concepts" panel instead, since there's nowhere here to click through to. | `TopNav`, `Meter`(`ui`), `useDocumentTitle`, `useProgress`, `data/concepts.ts`, `data/curriculum.ts` | Stage-2 chips link to `/shell-scripting` and `/rebase-history` — the page those two are linked *from*. |
