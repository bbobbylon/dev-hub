...
_Last updated: 2026-09-18 — describes the 33-page app plus the optional account/sync layer, on the
...
**Main features:**
- 33 standalone interactive lesson/tool pages spanning CLI/shell, Git, algorithms, data
...
- **Concept completion** — every lesson and practice page can record the concept it teaches
  (`data/concepts.ts` registers all 28, with the route that teaches each and the action that earns
...
- All 34 routes (33 lesson/tool pages + the gallery) render without console errors, at all three
  verified breakpoints — enforced by `npm run verify` before any deploy. A build configured against
  a backend has 36: the two account routes exist only then, and `scripts/routes.mjs` reads the same
