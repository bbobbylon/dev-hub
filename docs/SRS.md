...
- All 30 routes (29 lesson/tool pages + the gallery) render without console errors, at all three
  verified breakpoints — enforced by `npm run verify` before any deploy. A build configured against
  a backend has 32: the two account routes exist only then, and `scripts/routes.mjs` reads the same
