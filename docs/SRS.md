...
_Last updated: 2026-09-18 — describes the 32-page app plus the optional account/sync layer, on the
...
**Main features:**
- 32 standalone interactive lesson/tool pages spanning CLI/shell, Git, algorithms, data
...
- **Concept completion** — every lesson and practice page can record the concept it teaches
  (`data/concepts.ts` registers all 27, with the route that teaches each and the action that earns
...
- All 33 routes (32 lesson/tool pages + the gallery) render without console errors, at all three
  verified breakpoints — enforced by `npm run verify` before any deploy. A build configured against
  a backend has 35: the two account routes exist only then, and `scripts/routes.mjs` reads the same
